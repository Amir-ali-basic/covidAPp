<template>
  <div class="selectContainer">
    <select @change="changeCountry" v-model="selected">
  ...
  <div class="container grid" v-if="selected !== 0">
    <div>
      ...
      <div v-for="confirmed in arrConfirmed" :value="confirmed.ID">
        <div>
          {{ confirmed.date }} - {{ numberWithSpaces(confirmed.total) }}
        </div>
      </div>
    </div>
    ...
    <div>
      ...
      <div v-for="recovered in arrRecovered" :value="recovered.ID">
        <div>
          {{ recovered.date }} - {{ numberWithSpaces(recovered.total) }}
        </div>
      </div>
    </div>
    ...
    async fetchCovidDataByDay() {
      try {
        const res = await fetch(
          `https://api.covid19api.com/live/country/${this.name}/status/confirmed`
        );
        ...
        data.forEach((item, i) => {
          ...
          this.arrConfirmed
            .reverse()
            .push({ date, total: Confirmed - (data[i - 1]?.Confirmed || 0) });
          this.arrDeaths
            .reverse()
            .push({ date, total: Deaths - (data[i - 1]?.Deaths || 0) });
